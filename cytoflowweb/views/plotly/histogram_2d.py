"""2D Histogram view — np.histogram2d server-side, rendered as go.Heatmap."""
...
def render(experiment, params: dict) -> dict:
    ...
    for ri, rv in enumerate(row_vals):
        for ci, cv in enumerate(col_vals):
            ...
            # Transpose: histogram2d returns (nx, ny), go.Heatmap wants (ny, nx)
            fig.add_trace(
                go.Heatmap(
                    z=counts.T.tolist(),
    ...
    layout = dict(
        **BASE_LAYOUT,
        title={"text": f"{xchannel} vs {ychannel} (2D histogram)", "font": {"size": 14}},
